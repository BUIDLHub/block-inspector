...
var log = new _biUtils.Logger({ component: "LocalFSStorage" });

var homedir = require('os').homedir();
var baseDir = _path2.default.join(homedir, "bi-storage");
...
var LocalFSDriver = function () {
  function LocalFSDriver() {
    ...
    this.data = {};
    ['_initStorage', 'dropInstance', 'clear', 'getItem', 'iterate', 'key', 'keys', 'length', 'removeItem', 'setItem'].forEach(function (fn) {
  ...
  _createClass(LocalFSDriver, [{
    ...
    value: function _initStorage(options) {
      ...
      self.dbName = options.name;
      self.data = _read(options.name);
      return Promise.resolve();
    }
  }, {
    key: 'dropInstance',
    value: function dropInstance(callback) {
      var _this2 = this;

      log.info("Dropping local FS DB", this.dbInfo);
      var p = asProm(function () {

        _this2.data = {};
        _write(_this2.dbName, _this2.data);
      });
    ...
    value: function removeItem(key, callback) {
      ...
      var p = asProm(function () {
        if (_this9.data[key]) {
          ...
          log.debug("Removing item with key", key, _this9.data);
    ...
    value: function setItem(key, value, callback) {
      ...
      var p = asProm(function () {
        log.debug("Setting item", key, "on db", _this10.dbName);
